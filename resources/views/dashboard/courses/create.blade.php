...
@section('content')
    ...
    <h2 class="uk-title">@isset($item)Редактировать@elseСоздать@endisset курс ({{ $direction->title }})</h2>
    <form class="uk-form" enctype=multipart/form-data method="POST" action="{{ isset($item) ? route('course.update', $item->id) : route('course.store', $direction->id) }}">
        @isset($item)
            @method('PATCH')
        @endisset
        @csrf
        <div class="uk-flex">
            <div class="uk-width-1-3">
                ...
                <div class="uk-margin-small">
                    <input type="number" placeholder="Количество часов" name="length" class="uk-input">
                </div>
                <div class="uk-margin-small">
